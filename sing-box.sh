...
yellow "2：使用1.10系列之后最新正式内核"
readp "请选择1-2：" menu
if [ -z "$menu" ] || [ "$menu" = "1" ] ; then
    sbcore=$(curl -Ls https://data.jsdelivr.com/v1/package/gh/SagerNet/sing-box | grep -Eo '"1\.10[0-9\.]*",' | sed -n 1p | tr -d '",')
else
    # 仅选择 v1.12.0 及以上版本中的最新版本
    sbcore=$(curl -Ls https://data.jsdelivr.com/v1/package/gh/SagerNet/sing-box | grep -Eo '"(1\.(1[2-9]|[2-9][0-9])\.[0-9]+)",' | sed -n 1p | tr -d '",')
fi
