...
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
green "使用哪个内核版本？"
yellow "1：使用1.10系列正式版内核 (支持geosite分流)"
yellow "2：使用最新正式版内核"
yellow "3：使用最新beta版内核"
readp "请选择【1-3】：" menu
if [ -z "$menu" ] || [ "$menu" = "1" ]; then
    ...
    sbcore=$(curl -Ls https://data.jsdelivr.com/v1/package/gh/SagerNet/sing-box | grep -Eo '"[0-9.]+-beta\.[0-9]+",' | sed -n 1p | tr -d '",')
    ENABLE_ANYTLS=true
else
...
# 2. 添加 `anytlsport` 函数
# 在 `vlport`、`vmport` 等函数附近，添加以下函数
anytlsport(){
    readp "\n设置AnyTLS端口[1-65535] (回车跳过为10000-65535之间的随机端口)：" port
    chooseport
    port_anytls=$port
}

# 3. 修改 `insport` 函数
# 找到原始 `insport` 函数，替换为以下内容
insport(){
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
green "三、设置各个协议端口"
...
# 4. 修改 `inscertificate` 函数
# 在 `inscertificate` 函数中，找到 `ymzs` 和 `zqzs` 函数定义后，添加以下内容
if [ "$ENABLE_ANYTLS" = "true" ]; then
    certificatec_anytls=$certificatec_vmess_ws
    certificatep_anytls=$certificatep_vmess_ws
    ym_anytls=$ym_vm_ws
fi

# 5. 修改 `inssbjsonser` 函数
# 找到 `sb11.json` 的写入部分，替换为以下内容
